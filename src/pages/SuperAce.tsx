...
const SuperAce = () => {
  ...
  const startGame = async () => {
    ...
    setTimeout(() => {
      const shouldWin = newBetCount <= 2 || shouldBetWin(user?.id || 'anonymous', bet);
      
      let win = 0;
      if (shouldWin) {
        win = calculateWin(drawnCards);
      }
  ...
  const handleGameResult = (win) => {
    ...
    if (win > 0) {
      ...
      showSuccessToast(
        win >= bet * 5 ? "Big Win!" : "You Won!",
        `${win.toFixed(0)}৳ added to your balance`
      );
    } else {
      setResult('LOSE');
